<div>

  <img src="https://raw.githubusercontent.com/roscibely/algorithms-and-data-structure/main/root/Ufersa.png" width="70" height="100">
</div>

# PEX1241 - Algoritmos e Estrutura de dados I
###### Professor: Reudismam Rolim de Sousa

#### Bacharelado Interdisciplinar em Tecnologia da Informação - UFERSA

<p align="center">
- Repositório criado para armazenar os repositórios desenvolvidos na disciplina de Algoritmos e Estrutura de Dados I.
</p>

## 👨‍🎓 Discente

Matheus Almeida Rigueira

## 📅 Período

2026.1

## 📂 Estrutura do Repositório

📁 tarefas       → Resolução das tarefas desenvolvidas no Google Classroom
📁 projetos      → Projetos desenvolvidos na disciplina  
📁 provas        → Exercícios ou simulados de provas  


## 🚀 Objetivo

Este repositório tem como objetivo registrar o aprendizado na disciplina de Algoritmos e Estruturas de Dados I, reunindo implementações, resoluções de exercícios e estudos práticos sobre os principais conceitos da área.

Além de servir como acompanhamento da evolução acadêmica, também funciona como material de apoio para revisões futuras.

⭐ Caso queira acompanhar mais projetos, visite meu perfil no GitHub.
